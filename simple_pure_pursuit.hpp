...
class SimplePurePursuit : public rclcpp::Node {
 public:
  ...
  const double cte_proportional_gain_; // Stanley法用のゲインを追加
  const double k_softening; // Stanley法のソフトニング定数を追加

 private:
  void onTimer();
  bool subscribeMessageAvailable();
  // 以下の補助関数はonTimer内にロジックを統合したため不要
  // double computeCrossTrackError(const TrajectoryPoint &traj_point, const geometry_msgs::msg::Pose &pose);
  // double computeHeadingError(const TrajectoryPoint &traj_point, const TrajectoryPoint &prev_traj_point, const geometry_msgs::msg::Pose &pose);
};
